...
public:
    ...
    void obtenerRelaciones(int clave);
private:
...
template <typename T>
    ...
    template <typename T>
NodoAA<T>* ArbolAA<T>::encontrarPadre(NodoAA<T>* actual, NodoAA<T>* nodo) {
    if (actual == nullptr || actual == nodo) return nullptr;
    if (actual->izquierda == nodo || actual->derecha == nodo) return actual;

    NodoAA<T>* padreIzq = encontrarPadre(actual->izquierda, nodo);
    if (padreIzq) return padreIzq;

    return encontrarPadre(actual->derecha, nodo);
}

template <typename T>
NodoAA<T>* ArbolAA<T>::buscarNodo(NodoAA<T>* nodo, int clave) {
    if (!nodo || nodo->clave == clave) return nodo;
    if (clave < nodo->clave) return buscarNodo(nodo->izquierda, clave);
    return buscarNodo(nodo->derecha, clave);
}

template <typename T>
void ArbolAA<T>::obtenerRelaciones(int clave) {
    NodoAA<T>* nodo = buscarNodo(raiz, clave);
    if (!nodo) {
        std::cout << "Nodo con clave " << clave << " no encontrado." << std::endl;
        return;
    }

    NodoAA<T>* padre = encontrarPadre(raiz, nodo);
    NodoAA<T>* abuelo = padre ? encontrarPadre(raiz, padre) : nullptr;
    NodoAA<T>* hermano = (padre && padre->izquierda != nodo) ? padre->izquierda : (padre ? padre->derecha : nullptr);

    NodoAA<T>* hijoIzq = nodo->izquierda;
    NodoAA<T>* hijoDer = nodo->derecha;

    std::cout << "Relaciones del nodo con clave " << clave << ":" << std::endl;
    if (padre) std::cout << "Padre: " << padre->clave << std::endl;
    else std::cout << "Padre: No existe" << std::endl;

    if (abuelo) std::cout << "Abuelo: " << abuelo->clave << std::endl;
    else std::cout << "Abuelo: No existe" << std::endl;

    if (hermano) std::cout << "Hermano: " << hermano->clave << std::endl;
    else std::cout << "Hermano: No existe" << std::endl;

    if (hijoIzq) std::cout << "Hijo Izquierdo: " << hijoIzq->clave << std::endl;
    else std::cout << "Hijo Izquierdo: No existe" << std::endl;

    if (hijoDer) std::cout << "Hijo Derecho: " << hijoDer->clave << std::endl;
    else std::cout << "Hijo Derecho: No existe" << std::endl;
}
