...
void mostrarArbolAAPuestosOcupados(ListaCircularDoble<Coche>& listaCoches) {
    ArbolAA<int> arbolAA;
    arbolAA.insertarPuestosOcupados(listaCoches); 
    arbolAA.verArbol();  
    arbolAA.mostrar();  
    arbolAA.niveles();
    cout << endl << arbolAA.altura(arbolAA.raiz) << endl;
    cout << endl << arbolAA.profundidad(arbolAA.raiz->clave) << endl;
    arbolAA.obtenerRelaciones(11);
}
    ...
    while (!salirSubmenu)
    {
        ...
        case 2:
        {
           ...
           string cedula;
            do
            {
                cedula = validador.ingresarCedula("Ingrese la cédula a buscar: ");
                if (!validador.validarCedula(cedula))
                {
                    cout << "Cédula inválida. Intente de nuevo." << endl;
                }
            } while (!validador.validarCedula(cedula));
...
void menu(ListaCircularDoble<Coche> &lista, ListaCircularDoble<Coche> &listaHistorial, ListaPropietarios &listaPropietarios)
{
    ...
    vector<string> opciones = {
        ...
        "Ayuda",
        "Imprimir arbol",
        "Graficar arbol Puestos Ocupados",
    ...
    while (true)
    {
        ...
        case 5:
        {
            ...
            Nodo<Coche>* nodoCoche = lista.obtenerNodoPorPlaca(placa);

            if (nodoCoche != nullptr) {
                int puesto = nodoCoche->getDato().getPuesto(); 

                listaHistorial.salirDelParqueadero(placa);     
                lista.eliminarPorPlaca(placa);                 
                liberarPuesto(puesto);                         

                cout << "El coche con la placa " << placa << " ha salido del parqueadero y se ha liberado el puesto " << puesto << "." << endl;
            } else {
                cout << "El coche con la placa " << placa << " no se encuentra en el parqueadero." << endl;
            }
        ...
        case 9:
        {
                system("cls");
                cout << "========================================" << endl;
                cout << "   ÁRBOL AA DE PUESTOS OCUPADOS          " << endl;
                cout << "========================================" << endl;
                mostrarArbolAAPuestosOcupados(lista); 
                break;
        }
        case 10:
        {
