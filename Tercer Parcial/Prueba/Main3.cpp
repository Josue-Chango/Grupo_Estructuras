...
/*int main() {
    ...
    for (int size : sizes) {
        ...
        measureExecutionTime([&]() { objeto.insertIntoTree(size); }, "Insercion en Árbol Binario", size);
        measureExecutionTime([&]() { objeto.insertIntoTree(size); }, "Insercion en Árbol Binario", size);
        measureExecutionTime([&]() { ejercicio.verificarGeneral(size); }, "Insercion en Árbol Binario", size);
    }
    for (double n = 0.1; n <= 10; n += 0.1) {
        dataFile << n << "," << n_e(n) << "," << e_n(n) << "\n";

        // Medir tiempos de ejecución para ambas funciones
        measureExecutionTime([&]() { n_e(n); }, "n^e", n);
        measureExecutionTime([&]() { e_n(n); }, "e^n", n);
    }
...
int main() {
    ...
    // Generar datos en el rango de 0.1 a 10 con incrementos de 0.1
    ofstream dataFile("valores_funciones.csv");
    dataFile << "n,n^e,e^n\n"; // Encabezado
    Ejercicio2 objeto;
    double c, n0, max_n;

    std::cout << "Ingrese el valor de la constante de incremento: ";
    std::cin >> c;
    
    std::cout << "Ingrese el valor inicial n0: ";
    std::cin >> n0;
    
    std::cout << "Ingrese el valor maximo de n para la evaluacion: ";
    std::cin >> max_n;

    for (double n = n0; n <= max_n; n += c) {
        dataFile << n << "," << objeto.n_e(n) << "," << objeto.e_n(n) << "\n";

        // Medir tiempos de ejecución para ambas funciones
        measureExecutionTime([&]() { objeto.n_e(n); }, "n^e", n);
        measureExecutionTime([&]() { objeto.e_n(n); }, "e^n", n);
    }

    dataFile.close();

    cout << "Datos guardados en resultados.csv y valores_funciones.csv\n";
    ...
    if (matlabStatus != 0) {
        cout << "MATLAB no encontrado, intentando graficar con Python..." << endl;
        system("python analizar_complejidad.py");
    }
    system("pause");
    cout << "Presiona Enter para salir...";
    cin.get();
