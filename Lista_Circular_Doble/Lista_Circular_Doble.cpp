...
/*
template <typename T>
void Lista_Circular_Doble<T>::buscarEnRegistro( std::string horaInicio,  std::string horaFinal) {
    // Convertir las horas de cadena de caracteres a objeto time_t
    std::tm tmInicio = {};
    std::tm tmFinal = {};
    std::istringstream issInicio(horaInicio), issFinal(horaFinal);
    ...
    // Recorrer el registro y mostrar los datos que se encuentren dentro del rango de horas
    Nodo_Circular_Doble<T>* aux = this->cabeza;
    do {
        int hora = std::stoi(aux->getHora());
        int minuto = std::stoi(aux->getMinuto());
        int segundo = std::stoi(aux->getSegundo());

        std::tm tmRegistro = {};
        tmRegistro.tm_hour = hora;
        tmRegistro.tm_min = minuto;
        tmRegistro.tm_sec = segundo;
        time_t tRegistro = std::mktime(&tmRegistro);

        if (tRegistro >= tInicio && tRegistro <= tFinal) {
            std::cout << aux->getNombre1() << " " << aux->getNombre2() << " "
                      << aux->getApellido() << " " << aux->getCedula() << " "
                      << aux->getCorreo() << " " << aux->getPlaca() << ", "
                      << aux->getFecha() << ", "
...
/*template <typename T>
void Lista_Circular_Doble<T>::buscarEnRegistro(const std::string& horaInicio, const std::string& horaFinal) {
    // Validar el rango de horas
    if (!validarHora(horaInicio, horaFinal)) {
    ...
    do {
        try {
            int hora = std::stoi(actual->getHora());
            int minuto = std::stoi(actual->getMinuto());
            int segundo = std::stoi(actual->getSegundo());
            ...
            if (tRegistro >= tInicio && tRegistro <= tFinal) {
                std::cout << actual->getNombre1() << " " << actual->getNombre2() << " "
                          << actual->getApellido() << " " << actual->getCedula() << " "
                          << actual->getCorreo() << " " << actual->getPlaca() << ", "
                          << actual->getFecha() << ", "
                          << std::setw(2) << std::setfill('0') << hora << ":"
                          << std::setw(2) << std::setfill('0') << minuto << ":"
                          << std::setw(2) << std::setfill('0') << segundo << std::endl;
            ...
            // Si no se puede convertir a entero, simplemente omitir el registro
            std::cerr << "Error al procesar un registro: " << e.what() << std::endl;
        }
...
void Lista_Circular_Doble<T>::buscarEnRegistro(std::string hora_inicio, std::string hora_final)
{
    ...
    std::stringstream iss(hora_inicio);
    ...
    try {
        hora1 = std::stoi(horaStr);
        minuto1 = std::stoi(minutoStr);
        segundo1 = std::stoi(segundoStr);
    } catch (const std::invalid_argument& e) {
        // Si no se puede convertir a entero, establecer valores predeterminados
        hora1 = 0;
        minuto1 = 0;
        segundo1 = 0;
        std::cerr << "Error al descomponer la hora: " << e.what() << std::endl;
    }

    int hora2, minuto2, segundo2;
    std::stringstream iss2(hora_final);
    std::getline(iss2, horaStr, ':');
    std::getline(iss2, minutoStr, ':');
    std::getline(iss, segundoStr, ':');

    try {
        hora2 = std::stoi(horaStr);
        minuto2 = std::stoi(minutoStr);
        segundo2 = std::stoi(segundoStr);
    } catch (const std::invalid_argument& e) {
        // Si no se puede convertir a entero, establecer valores predeterminados
        hora2 = 0;
        minuto2 = 0;
	...
	if (this->cabeza != nullptr)
	{
		...
		do
		{
			if (stoi(aux->getHora()) >= hora1  && stoi(aux->getHora()) <= hora2 && stoi(aux->getMinuto()) >= minuto1  && stoi(aux->getMinuto()) <= minuto2 && stoi(aux->getSegundo()) >= segundo1  && stoi(aux->getSegundo()) <= segundo2)
			{
				cout << aux->getNombre1() << " " << aux->getNombre2() << " " << aux->getApellido() << " " << aux->getCedula() << " " << aux->getCorreo() << " " << aux->getPlaca() << "," << aux->getFecha() << "," << aux->getHora() << "," << aux->getMinuto() << "," << aux->getSegundo() << endl;
			}
			aux = aux->getSiguiente();
