...
int ingresar(char* msj) {
    ...
    while ((c = getch()) != 13) {  // 13 es el código ASCII de Enter
        if (c >= '0' && c <= '9') {  // Solo se aceptan dígitos
            printf("%c", c);
            cad[i++] = c;
    ...
    valor = atoi(cad);  // Conversión a entero
    return valor;
}
...
float ingresarFloat(char* msj) {
    ...
    cad[i] = '\0';  // Terminador de cadena
    return atof(cad);  // Conversión a flotante
}
...
double ingresarDouble(char* msj) {
    ...
    printf("%s", msj);
    while ((c = getch()) != 13) {  // 13 es el código ASCII de Enter
        if (c >= '0' && c <= '9') {  // Acepta dígitos
            printf("%c", c);
            cad[i++] = c;
        } else if (c == 8 && i > 0) {  // Backspace
            printf("\b \b");
            i--;
        } else if (c == '.' && !puntoDecimal) {  // Acepta un único punto decimal
            puntoDecimal = true;
            printf("%c", c);
            cad[i++] = c;
        }
    ...
    cad[i] = '\0';  // Terminador de cadena
    valor = atof(cad);  // Conversión a double
    return valor;
}
