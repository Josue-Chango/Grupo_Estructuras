...
template <typename T>
Nodo<T>* ListaCircularDoble<T>::obtenerNodoPorPlaca(string placa) {
    if (primero == nullptr) {
    ...
    Nodo<T>* temp = primero;
    do {
        if (temp->getDato().getPlaca() == placa) {
            return temp;  
        }
        temp = temp->getSiguiente();
    } while (temp != primero);

    cout << "Coche con placa " << placa << " no encontrado." << endl;
    return nullptr;  
}
